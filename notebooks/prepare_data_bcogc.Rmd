...
```{R}
all_wells_sf <- dplyr::bind_rows(horiz_dev_well_sf, vert_well_sf) %>%
...
data.frame(well_geology_df) %>%
  mutate_all(., function(x) as.numeric(x)) %>%
  cbind(
    all_wells_sf %>% as.data.frame() %>% select(unique_surv_id, wa_num),.
    ) %>%
  write_csv(., paste0('../output/','bcogc_well_geology_df.csv'))
```
...
```{R}
well_geology_df <- read.csv(paste0('../output/','bcogc_well_geology_df.csv'))
...
dist <- c(1000,3000,5000,7000,9000)
mag <- c(0.25,0.75,1.25,1.75,2.25)
npts <- c(3,5,7,9,11)
grid = expand.grid(dist, mag, npts) %>%
  rename(distance="Var1",magnitude="Var2", minpts="Var3")
...
for (row in 1:nrow(grid)){
  ...
  seismic_catalog <- seismic_catalog %>%
    hdbscan_seismic_catalog(., npts = grid[row,'minpts'])
  ...
  seismogenic_wells = append(seismogenic_wells, seis_sum)
...
saf_wells_sf %>% 
  ...
  sf::st_drop_geometry() %>%
  data.table::fwrite(., paste0('../output/','bcogc_saf_well_data.csv'))

write_rds(saf_wells_sf, paste0('../output/','bcogc_saf_well_data.rds'))
```