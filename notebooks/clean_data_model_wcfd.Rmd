...
ml_df <- saf_wells_sf %>%
  st_drop_geometry() %>% 
  dplyr::select(
    all_of(aux_cols), all_of(class_comp_feats), all_of(class_geo_feats), 
    all_of(regr_comp_feats), all_of(regr_geo_feats), all_of(targets)
    ) %>%
...
```{r}
write_csv(ml_df, paste0("../wcfd_data/","final_wcfd_mldf.csv"))
write_rds(ml_df, paste0("../wcfd_data/","final_wcfd_mldf.rds"))
```