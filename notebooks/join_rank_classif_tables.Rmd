...
```{r}
files = list.files(
  path = "../output/classif_feat_summary/", 
  pattern=paste0('filter_feats.csv'),
  full.names = TRUE)

for (f in files){
  header = f %>%
    str_replace(.,"../output/classif_feat_summary/classif_","") %>%
...
filter_feats %>%
  ...
  write_csv('../output/classif_filter_values.csv')
...
rank <- filter_feats %>% 
  ...
  mutate(feat = filter_feats$feat) %>%
  select('feat',everything()) %>% 
  write_csv('../output/classif_filter_rank.csv')
```
...
seq_feats %>%
  ...
  select('feat',everything()) %>%
  write_csv('../output/classif_seq_values.csv')
...
seq_feats %>% 
  ...
  mutate(feat = seq_feats$var) %>%
  select('feat',everything()) %>% 
  write_csv('../output/classif_seq_rank.csv')
```
...
rand_feats %>%
  ...
  select('feat',everything()) %>%
  write_csv('../output/classif_rand_values.csv')
...
rand_feats %>% 
  ...
  mutate(feat = rand_feats$var) %>%
  select('feat',everything()) %>% 
  write_csv('../output/classif_rand_rank.csv')
```

Join all tables and final mean rank

```{r}
filter <- read_csv('../output/classif_filter_rank.csv') %>%
  select(feat, filter_mean_rank)

seq <- read_csv('../output/classif_seq_rank.csv') %>%
  select(feat, seq_mean_rank)

rand <- read_csv('../output/classif_rand_rank.csv') %>%
...
final <- filter %>%
  ...
  select(feat, everything()) %>%
  write_csv('../output/classif_final_rank.csv')
```
