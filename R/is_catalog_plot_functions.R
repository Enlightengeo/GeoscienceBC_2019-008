...
train_aki_model <- function(ecdf, cat){
  ...
  loss_func <- function(a,b,ecdf){
    yardstick::mae_vec(b*ecdf$centres + a, ecdf$log_cdf)
  }
  
  # Use a vector of a values and determine best mae / MAE value
...
train_gof_model <- function(ecdf){
  loss_func <- function(par,ecdf){
    ...
    yardstick::mae_vec(par[2]*ecdf_filt$centres + par[3], ecdf_filt$log_cdf)
  }
...
run_plot_models <- function(ecdf, cat, plot = FALSE){
 ...
 out_df = out_df %>%
   dplyr::mutate(weight = 1/(mae / mean(out_df$mae))) %>%
...
b_value_plot <- function(ecdf,cat,out_df){
  aki = out_df[out_df$method == 'aki',]
  maxc = out_df[out_df$method == 'maxc',]
  gof = out_df[out_df$method == 'gof',]
  
  # Import a discrete colour vector for lines
  col_vect <- brewer.pal(5,'Set1')
  aki_text <- paste('ML - Mc: ',sprintf("%.1f", round(aki$mmax_c,2)),
                    ' b: ',sprintf("%.2f", -round(aki$b,2)),
                    ' MAE: ',sprintf("%.2f", round(aki$mae,2),sep=''))
  gof_text <- paste('GOF - Mc: ',sprintf("%.1f", round(gof$mmax_c,2)),
                    ' b: ',sprintf("%.2f", -round(gof$b,2)),
                    ' MAE: ',sprintf("%.2f", round(gof$mae,2),sep=''))
  maxc_text <- paste('MC - Mc: ',sprintf("%.1f", round(maxc$mmax_c,2)),
                     ' b: ',sprintf("%.2f", -round(maxc$b,2)),
                     ' MAE: ',sprintf("%.2f", round(maxc$mae,2),sep=''))
  ...
  # B-value plot 
  ggplot(cat, aes(mw)) +
    geom_histogram(aes(mw), binwidth = gof$bin_width*3) +
    geom_abline(aes(intercept = aki$a, slope = aki$b, col = 'Maximum\nLikelihood (ML)\n'), size = 1) +
    geom_text(aes(x, y, label = aki_text), data = data.frame(x = xrng[2], y = yrng[2]), 
              hjust = 1, vjust = 0, size = 3.25, col = col_vect[1]) +
    geom_abline(aes(intercept = gof$a, slope = gof$b, col = 'Goodness\nof Fit (GOF)\n'), size = 1) +
    geom_text(aes(x, y, label = gof_text), data = data.frame(x = xrng[2], y = yrng[2]), 
              hjust = 1, vjust = 2, size = 3.25, col = col_vect[2]) +
    geom_abline(aes(intercept = maxc$a, slope = maxc$b, col = 'Maximum\nCurvature (MC)\n'), size = 1) +
    ...
    ggsave('b_value_plot.jpeg', width = 6, height = 6, dpi = 600) +
    ggsave('b_value_plot.pdf', width = 6, height = 6)
}
